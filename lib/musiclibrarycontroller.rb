class MusicLibraryController
    ...
    def list_artists
        Artist.all.sort_by{|artist| artist.name}.uniq.each_with_index{|element, index| puts "#{index + 1}. #{element.name}"}  
    end 
    
    def list_genres
        Genre.all.sort_by{|genre| genre.name}.uniq.each_with_index{|element, index| puts "#{index + 1}. #{element.name}"}  
    end 
